...
class UserService {
  async reg(email, password) {
    ...
    await mailServices.sendActivationMail(email, `${process.env.API_URL}/activate/${activationLink}`);
  ...
  async login(email, password) {
    ...
    if (!user) {
      throw ApiError.BadRequest('ОШИБКА: Пользователь с таким E-mail не найден!');
    }
